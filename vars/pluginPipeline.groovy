                    ...
                    dir ('gerrit') {
                        script {
                            ([pluginName] + extraPlugins + extraModules + extraGhRepos).each { plugin ->
                                sh "cd plugins && ln -s ../../${plugin} ."
                                sh "if [ -f ../${plugin}/external_plugin_deps.bzl ]; then cd plugins && cat ${plugin}/external_plugin_deps.bzl >> external_plugin_deps.bzl; fi"
                                sh "if [ -f ../${plugin}/external_plugin_deps.MODULE.bazel ]; then cd plugins && cat ${plugin}/external_plugin_deps.MODULE.bazel >> external_plugin_deps.MODULE.bazel; fi"
                                sh "if [ -f ../${plugin}/external_package.json ]; then cd plugins && cat ${plugin}/external_package.json >> package.json; fi"
                                sh "cat plugins/external*"
                            }
                        }
